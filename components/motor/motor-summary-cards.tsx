...
import type { MotorSummaryStat } from "@/lib/motor-data"
import { cn } from "@/lib/utils"

const iconMap: Record<MotorSummaryStat["icon"], LucideIcon> = { clock: Clock, power: Power, starts: RotateCw, water: Droplet }
...
export function MotorSummaryCards({ stats }: { stats: MotorSummaryStat[] }) {
  return (
    <section className="rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {stats.map((stat) => <StatCard key={`${stat.motorId}-${stat.label}`} stat={stat} />)}
      </div>
    </section>
  )
}
