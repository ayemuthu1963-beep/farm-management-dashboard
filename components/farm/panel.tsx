...
interface PanelProps {
  ...
  /** optional content aligned to the right side of the panel header */
...
export function Panel({ title, icon: Icon, iconClassName, headerRight, children, className, bodyClassName }: PanelProps) {
  return (
    <section className={cn("rounded-xl border border-border bg-card shadow-sm", className)}>
      <div className="flex items-center gap-2 border-b border-border px-4 py-3 sm:px-5">
        ...
        {headerRight ? <div className="ml-auto text-right">{headerRight}</div> : null}
      </div>
