...
import {
  fetchTreeWiseQueryData,
  HarvestApiError,
  type TreeWiseQueryFilters,
} from "@/lib/coconut-harvest-api"

function readFilters(request: Request): TreeWiseQueryFilters {
  ...
  return {
    ...
    includeNoRecord: searchParams.get("includeNoRecord") !== "false",
  }
}
...
export async function GET(request: Request) {
  ...
  try {
    const data = await fetchTreeWiseQueryData(readFilters(request))
    ...
    console.error(`[tree-wise-query:${referenceId}]`, error)
    return NextResponse.json(
      { error: "Tree-wise Table Query could not be completed", referenceId },
