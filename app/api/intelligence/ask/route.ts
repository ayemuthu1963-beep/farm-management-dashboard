...
function approvedEnvironment() {
  ...
  const database = (process.env.MFMS_TARGET_DATABASE ?? "").trim()
  const expectedDatabase = environment === "production" ? "mfms_server_prod"
    : environment === "preview" || environment === "uat" ? "mfms_server_uat" : null
  if (!expectedDatabase || database !== expectedDatabase) return false
  const publicEnvironment = process.env.NEXT_PUBLIC_MFMS_ENV?.trim().toLowerCase()
  const publicDatabase = process.env.NEXT_PUBLIC_MFMS_TARGET_DATABASE?.trim()
  const publicDatabaseLabel = process.env.NEXT_PUBLIC_MFMS_ENV_DATABASE_LABEL?.trim()
  if (publicEnvironment !== undefined && (environment === "production"
    ? publicEnvironment !== "production" : publicEnvironment !== "preview" && publicEnvironment !== "uat")) return false
  return (publicDatabase === undefined || publicDatabase === expectedDatabase)
    && (publicDatabaseLabel === undefined || publicDatabaseLabel === expectedDatabase)
}

function approvedBackendTarget(): URL | null {
  const expected = process.env.MFMS_ENV?.trim().toLowerCase() === "production"
    ? "http://harvest-api:8000" : "http://harvest-api-pilot:8000"
  const configured = process.env.HARVEST_API_BASE_URL
  if (configured !== expected && configured !== `${expected}/`) return null
  return new URL("/api/intelligence/ask", expected)
}

async function readBoundedPayload(request: NextRequest): Promise<{ payload?: unknown; tooLarge?: boolean }> {
...
export async function POST(request: NextRequest) {
  if (!approvedEnvironment()) return safeError(403, "MFMS Intelligence is not enabled for this environment and database.")
  const target = approvedBackendTarget()
  if (!target) return safeError(403, "MFMS Intelligence is not enabled for this backend target.")
  const contentLength = Number(request.headers.get("content-length") ?? "0")
  if (!Number.isSafeInteger(contentLength) || contentLength < 0 || contentLength > MAX_REQUEST_BYTES) return safeError(413, "The request is too large.")
  const { payload: rawPayload, tooLarge } = await readBoundedPayload(request)
  if (tooLarge) return safeError(413, "The request is too large.")
