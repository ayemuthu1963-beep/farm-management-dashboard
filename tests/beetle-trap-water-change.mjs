...
assert.match(dashboard, /Pheromone Change Date \(Cumulative Count Start Date\)/)
assert.match(dashboard, /bg-destructive\/15/)
assert.match(dashboard, /colSpan=\{4\}/)
assert.match(dashboard, /dailyTableRows\(data, rows, cumulativeStartDate\)/)
assert.match(chart, /dataKey="plot1RedPalmWeevil" name="Plot 1 — Red Palm Weevil \(solid\)"/)
assert.match(chart, /dataKey="plot1Rhinoceros" name="Plot 1 — Rhinoceros Beetle \(solid\)"/)
assert.match(chart, /dataKey="plot2RedPalmWeevil" name="Plot 2 — Red Palm Weevil \(dashed\)"/)
assert.match(chart, /dataKey="plot2Rhinoceros" name="Plot 2 — Rhinoceros Beetle \(dashed\)"/)
...
assert.match(chart, /x=\{date\} stroke="#047857" strokeWidth=\{3\}/)
assert.match(chart, /x=\{pheromoneChangeDate\} stroke="#dc2626" strokeWidth=\{3\}/)
assert.match(chart, /Pheromone change \/ reset/)
...
assert.match(dashboard, /waterChangeDates=\{waterChangeDates\} pheromoneChangeDate=\{cumulativeStartDate\}/)
