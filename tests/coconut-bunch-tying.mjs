...
assert.match(admin, /activeTab === "coverage" && selectedRoundId !== null/)
...
assert.match(api, /baseTreeNo >= 1 && baseTreeNo <= 999\) return "Plot 1"/)
assert.match(api, /baseTreeNo >= 1001\) return "Plot 2"/)
...
assert.match(detailed, /Number\.NEGATIVE_INFINITY/)
