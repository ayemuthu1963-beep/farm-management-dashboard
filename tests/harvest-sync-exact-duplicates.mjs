...
const locationMap = read("components/admin/harvest-location-comparison-map.tsx")
...
assert.match(workspace, /status\?\.formId \?\? previewOdkForms\.harvest\.formId/)
...
assert.match(review, /const decision: ConflictDecisionAction/)
assert.match(review, /REASSIGN_SUBMISSION_TREE/)
assert.match(review, /Different trees — correct TreeNo and retain both/)
assert.match(review, /Tree number entered incorrectly/)
assert.match(review, /Both valid submissions will be retained/)
assert.match(review, /View submissions on map/)
assert.match(review, /HarvestLocationComparisonMap/)
assert.match(review, /submissionLocationLabel/)
assert.match(review, /row\.gps_accuracy_m == null/)
assert.match(model, /gps_latitude\?: number \| null/)
assert.match(model, /gps_longitude\?: number \| null/)
assert.match(model, /gps_accuracy_m\?: number \| null/)
assert.match(model, /gps_captured_at\?: string \| null/)
assert.match(locationMap, /FarmOrthomosaicMap/)
assert.match(locationMap, /farmCombinedLayer\.coordinatesUrl/)
assert.match(locationMap, /Distance between \{located\[0\]\?\.label/)
assert.match(locationMap, /No submitted location is available/)
assert.match(locationMap, /radius: item\.accuracy/)
assert.match(locationMap, /accuracyCircle\.getBounds\(\)/)
assert.match(locationMap, /\.map\(\(item, index\) =>/)
assert.match(locationMap, /value === null \|\| value === undefined \|\| value === ""/)
...
assert.match(proxy, /MfmsAdminIdentityError/)
