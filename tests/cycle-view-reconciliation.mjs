...
test("Cycle filter and all-future-cycle modes use the backend reconciliation endpoint", () => {
  ...
  assert.match(page, /const newestCycle = allCycleOptions\[0\]/)
  assert.match(page, /const newestCycleRow = harvestCycleRows\.find\(\(row\) => row\.cycle === newestCycle\)/)
  assert.match(page, /setCycle\(String\(newestCycle\)\)/)
  assert.match(page, /setSummaryUnavailableCycle\(newestCycle\)/)
  assert.match(page, /setDisplaySummary\(emptySummary\)/)
