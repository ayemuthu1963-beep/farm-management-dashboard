...
assert.ok(editor.includes("export function IrrigationPipelineEditor"), "the existing pipeline editor implementation remains available")
assert.ok(farmMap.includes("<FarmMapOrthomosaic"), "the release uses the same isolated three-crop map in Preview and Production")
assert.ok(!farmMap.includes("<IrrigationPipelineEditor"), "this crop map release does not enable pipeline editing")
