...
const rollbackRecord = readText("scripts/production-backend-rollback-record.py")
...
assert.match(gate, /readonly approved_log_driver="json-file"/)
assert.match(gate, /readonly approved_log_max_size="20m"/)
assert.match(gate, /readonly approved_log_max_file="5"/)
assert.equal((gate.match(/--log-driver "\$approved_log_driver"/g) ?? []).length, 3)
assert.equal((gate.match(/--log-opt "max-size=\$approved_log_max_size"/g) ?? []).length, 3)
assert.equal((gate.match(/--log-opt "max-file=\$approved_log_max_file"/g) ?? []).length, 3)
...
assert.match(restoreOriginalFunction, /replacement_id=\$\(rollback_record replacement-ready/)
assert.match(restoreOriginalFunction, /\[\[ "\$replacement_id" =~ \^\[0-9a-f\]\{64\}\$ && "\$replacement_id" == "\$live_id" \]\]/)
assert.match(restoreOriginalFunction, /disconnect_production_network "\$backend_live_container" target "\$replacement_id"[\s\S]*?docker inspect --format '\{\{\.Id\}\}'[\s\S]*?docker rm -f "\$replacement_id"/)
assert.match(rollbackRecord, /def prepare_only_replacement\(self, identity, operation, container\):/)
assert.match(rollbackRecord, /return self\.prepare_only_replacement\(identity, operation, container\)\["static"\]\["container_id"\]/)
assert.match(rollbackRecord, /self\.match_source\(observed, expected\) if role == "source" else self\.match_static\(observed, expected\)/)
