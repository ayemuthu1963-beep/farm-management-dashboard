...
assert.match(gate, /docker volume create/)
assert.match(gate, /Production backup restore volume labels are invalid/)
assert.match(gate, /Production backup restore volume is not empty/)
assert.match(gate, /type=volume,source=\$database_backup_restore_volume,target=\/var\/lib\/postgresql\/data,volume-nocopy/)
assert.doesNotMatch(gate, /--tmpfs \/var\/lib\/postgresql\/data/)
...
assert.ok(restoreFunction.indexOf("docker volume create") < restoreFunction.indexOf('docker run -d'))
assert.ok(restoreFunction.indexOf('docker rm -f "$database_backup_restore_container"') < restoreFunction.indexOf('docker volume rm "$database_backup_restore_volume"'))
...
assert.match(cleanupFunction, /docker volume rm -f "\$database_backup_restore_volume"/)
