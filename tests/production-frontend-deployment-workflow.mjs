...
assert.equal((helper.match(/--log-driver "\$approved_log_driver"/g) ?? []).length, 2)
assert.equal((helper.match(/--log-opt "max-size=\$approved_log_max_size"/g) ?? []).length, 2)
assert.equal((helper.match(/--log-opt "max-file=\$approved_log_max_file"/g) ?? []).length, 2)
...
assert.match(helper, /local preview_path git_fetch_stderr git_fetch_status sanitized_stderr/)
assert.match(helper, /local -a preview_contract_lines provenance_refs/)
assert.match(helper, /provenance_refs=\("\$preview_approved_revision" "\$preview_feature_revision"\)/)
assert.match(helper, /provenance_refs\+=\("\$coordinated_preview_merge_base"\)/)
assert.match(helper, /GIT_COMMAND_ERROR=command=fetch-preview-provenance status=\$git_fetch_status stderr=\$sanitized_stderr/)
...
assert.match(preflightFunction, /local backend_revision/)
assert.match(preflightFunction, /backend_revision=\$\(image_revision_for_container "\$backend_container"\)/)
assert.match(preflightFunction, /preflight_deployment_kind=\$preview_deployment_kind/)
assert.match(preflightFunction, /preflight_preview_approved_revision=\$preview_approved_revision/)
assert.match(preflightFunction, /preflight_preview_approved_image_id=\$preview_approved_image_id/)
assert.match(preflightFunction, /preflight_preview_feature_revision=\$preview_feature_revision/)
assert.match(preflightFunction, /preflight_preview_verified_file_count=\$preview_verified_file_count/)
assert.doesNotMatch(preflightFunction, /approved only for the exact fertiliser candidate/)
