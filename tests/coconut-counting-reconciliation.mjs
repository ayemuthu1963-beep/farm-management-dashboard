...
test("Production Harvested writes require source approval and exact Production identity", () => {
  ...
  // Production writes are enabled only because the Production adaptation
  // carries the exact source-controlled approval token and the runtime
  // identity and target-safety checks also pass.
  assert.equal(gate({ runtime: safeProductionWriteEnvironment, targetSafetyErrors: safeProductionErrors }), true)
  assert.equal(gate({
  ...
  assert.equal(gate({
    runtime: safeProductionWriteEnvironment,
    targetSafetyErrors: safeProductionErrors,
    sourceProductionApproval: "PREVIEW_SOURCE_POLICY__PRODUCTION_HARVESTED_WRITES_DISABLED",
  }), false)
